...
Arun::fnstart(nlppp);													// 11/24/02 AM.
...
//			errOut(false);
	...
	if (nlppp->fail_)															// 06/10/00 AM.
		{
		Arun::ret(nlppp,0,(RFASem*)0);	// Handle local vars!	// 11/24/02 AM.
...
if (!PostRFA::args_range(_T("singler"), args, nlppp->collect_, 0,
...
// 11/10/99 AM. Not an error for sem to be built at this point.
if (!PostRFA::args_range(_T("singlex"), args, nlppp->collect_, 0,
										/*DU*/ nfirst, nlast))
	return false;
...
if (!PostRFA::args_range(_T("merger"), args, nlppp->collect_, 0,
										/*DU*/ nlppp->first_,
												 nlppp->last_))
	return false;
...
for (pcoll = nodes; pcoll; pcoll = pcoll->Right())
	...
	_TCHAR *ren;
	ren = elt->getRename();
	_TCHAR *gp = elt->getGroup();											// 09/23/00 AM.
//	if (ren && Debug())
...
if (!Arg::str1(_T("postXrename"), (DELTS*&)args, name))
	return false;
if (!Arg::num1(_T("postXrename"), (DELTS*&)args, ord, true))
	ord = 0;			// Allowing 2nd arg to be optional.
						// In case of error, this is a reasonable recovery.
else if (!Arg::done(args, _T("postXrename"),nlppp->parse_))
	return false;
...
// Set context variable to value.
Var::setVal(xvar1, xval, /*DU*/ xdlist);
xpn->setDsem(xdlist);
