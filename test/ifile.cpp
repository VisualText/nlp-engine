...
if (codes_)
	...
	*fcode << _T("Arun::fnstart(nlppp);");									// 11/19/02 AM.
	Gen::nl(fcode);															// 04/04/03 AM.
	...
	// Handle L() local vars in @CODE region!							// 11/19/02 AM.
	*fcode << _T("Arun::ret(nlppp,0,(RFASem*)0);");						// 11/19/02 AM.
	Gen::nl(fcode);															// 04/04/03 AM.
...
_t_ofstream *fcode = gen->fcode_;
_t_ofstream *fhead = gen->fhead_;
_t_ofstream *fdata = gen->fdata_;											// 06/21/00 AM.
int id = gen->id_;
_TCHAR *algo = parse->getAlgo();											// 05/31/00 AM.
