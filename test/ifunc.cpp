...
// In this way, we manage the "stack" of local variables.
...
for (; darg; darg = darg->Right())
	...
	*fcode << _T("{");
	Gen::nl(fcode);															// 04/04/03 AM.
	*fcode << _T("void *loc = Arun::fnstart(nlppp);");					// 03/11/02 AM.
	Gen::nl(fcode);															// 04/04/03 AM.
	...
	*fcode << _T("return Arun::ret(nlppp,loc,(long)0);");				// 03/11/02 AM.
	Gen::nl(fcode);															// 04/04/03 AM.
	*fcode << _T("}");
	Gen::nl(fcode);															// 04/04/03 AM.
