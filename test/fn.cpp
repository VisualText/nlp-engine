...
long val = 0;
if (!cg->findVal(conc1, attr_str, /*UP*/ val))			// FIX.	// 08/21/00 AM.
...
if (!(str = cg->strVal(conc1,attr_str)))					// OPT.	// 11/12/00 AM.
	{
...
if (!Arg::str1(																// 08/12/00 AM.
	...
	return false;
if (!Arg::any1(_T("replaceval"), (DELTS*&)args, /*UP*/ str, num, val_sem))	// 09/07/00 AM.
	return false;
...
if (!Arg::any1(_T("str"),/*DU*/(DELTS*&)args,typ,str,num,flt,sem1))			// 11/27/02 AM.
	return false;
...
if (!rename)																	// 10/30/00 AM.
	{
	_stprintf(Errbuf,_T("[attrexists: Warning. Given no new name.]"));// 05/18/01 AM.
	return parse->errOut(true); // UNFIXED 														// 05/18/01 AM.
	}
...
if (!Arg::any1(_T("str"),/*DU*/(DELTS*&)args,typ,str,num,flt,sem1))			// 05/29/02 AM.
...
if (!Arg::any1(_T("num"),/*DU*/(DELTS*&)args,typ,str,num,flt,sem1))			// 05/29/02 AM.
...
if (!Arg::any1(_T("arraylength"), (DELTS*&)args, str1,num1,sem1))
	return false;
...
if (!Arg::num1(_T("group"), (DELTS*&)args, num2))
	return false;
if (!Arg::str1(_T("group"), /*UP*/ (DELTS*&)args, name1))
	return false;
...
if (Pat::groupElts(coll1,coll2,node1,node2,name1,nlppp))
	return true;
return false;
}
...
//	case REGDECL:																// 03/12/02 AM.
		break;
	default:
		_stprintf(Errbuf,_T("[exitpass: Only implemented in @CODE and @DECL. Ignoring.]"));
		return parse->errOut(true); // UNFIXED 													// 05/18/01 AM.
	}
...
// Allow empty arg.
if (args)
	Arg::any1(_T("pncopyvars"), (DELTS*&)args, /*UP*/ str1, num, sem1);
if (args)																		// 11/13/01 AM.
...
if (!Arg::sem1(_T("pnmakevar"),nlppp,(DELTS*&)args,sem1))
	return false;
if (!Arg::str1(_T("pnmakevar"), /*UP*/ (DELTS*&)args, name1))
	return false;
if (!Arg::any1(_T("pnmakevar"), (DELTS*&)args, strval,numval,semval))
	return false;
...
if (semval)
	...
	if (semval->getType() == RSARGS)							// FIX.	// 08/07/03 AM.
		...
		{
		RFASem *cpsem = semval->copy_rfasem();
		Ivar::nodeReplaceval(pn, name1, cpsem);
		}
	}
else if (strval && *strval)
	Ivar::nodeReplaceval(pn, name1, strval);
else	// Numeric.
...
if (!Arg::str1(_T("pnreplaceval"), /*UP*/ (DELTS*&)args, name1))
	return false;
if (!Arg::any1(_T("pnreplaceval"), (DELTS*&)args, strval,numval,semval))
	return false;
...
//		Ivar::nodeReplaceval(pn, name1, cpsem);						// 11/14/02 AM.
		Ivar::nodeReplaceval(pn, name1, xargs);			// FIX.	// 11/14/02 AM.
		}
	else																			// 09/23/02 AM.
		{
		RFASem *cpsem = semval->copy_rfasem();
