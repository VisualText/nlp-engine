...
delete name_sem;
return findhierconcept(nlppp,name_sem,hsem);
}
...
// VARIANTS.																	// 04/28/01 AM.
_TCHAR *Arun::_tcschr(															// 04/28/01 AM.
	Nlppp *nlppp,
	RFASem *name1_sem,
	_TCHAR *ch_str
	)
...
_TCHAR *name1 = sem_to_str(name1_sem);
delete name1_sem;
return _tcschr(nlppp,name1,ch_str);
}
...
_TCHAR *name = name_sem->sem_to_str();
delete name_sem;
return _tcsrchr(nlppp,name,ch_str);
}
...
delete ch_sem;
return _tcsrchr(nlppp,name1,ch_str);
}
...
{
if (!nlppp || !oldStr || !*oldStr || !newStr || !*newStr)
	return 0;
...
{
void *savelocals = (void *) nlppp->locals_;
...
if (sem1 && sem1->getType() == RSARGS)					// FIX.	// 08/07/03 AM.
	...
	Ivar::nodeReplaceval(pn, name1, sem1);
...
if (strval && *strval)										// FIX.	// 07/06/06 AM.
	...
	Ivar::nodeReplaceval(pn,name1,(long)0);			// FIX.	// 07/06/06 AM.
