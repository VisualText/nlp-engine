...
//		stubname = passname + 5;		// Skip past "stub_".
		...
		if (tree = findStubconcept(stubname, gram))
			...
			genTree(tree, justDirty, skipLocks, /*DU*/ seq);	// 06/23/99 AM.
...
//	ok = genxConcept(tree, /*DU*/ seq);				// 06/26/99 AM.
	...
	// Traverse children.
	genTreeRec(cg_->Down(tree), false, &nmode, justDirty,skipLocks,
										 /*DU*/ seq);
...
if (!flag && !cg_->Prev(tree))
	...
	for (conc = cg_->Next(tree); conc; conc = cg_->Next(conc))
		genTreeRec(conc, false, mode, justDirty,skipLocks, /*DU*/ seq);
	}
return ok;
}
...
//sample = cg_->Next(sample))
	...
	else				// Partially parse the document.
		{
