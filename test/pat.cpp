...
if (!nlppp->node_->Left()											// 08/10/03 AM.
	...
	if (!(elt = elt1->Right()))									// 08/10/03 AM.
		...
		gerrStr << _T("[Rule ending with _xSTART not allowed.]") << ends;
		errOut(&gerrStr,false);													// 08/10/03 AM.
...
for (delt = list->getFirst(); delt; delt = delt->Right())
	{
	if (modeMatch1(nname, typ, delt->getData()->getStr(), deaccent))
		return true;
	}
return false;
}
...
if (treeExcept(elist,htab,node->pDown,deaccent,false))		// RECURSIVE.
	return true;
...
if (!root && !(node->pLeft))		// First node in list takes care of list.
	...
	for (node = node->pRight; node; node = node->pRight)
		{
		if (treeExcept(elist,htab,node,deaccent,false))
			return true;
		}
	}
return false;
...
inline bool Pat::warnWild(Irule *rule)
{
...
while (!done)
	...
	if ((rules)(ruleno, done, nlppp))	// If rule matched and actions done.
		...
		if (!nlppp->first_
			...
			parse->line_ = nlppp->rule_->getLine();					// 12/18/03 AM.
