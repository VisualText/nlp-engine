...
@POST
  ...
  pncopyvars(L("n"));
  L("last") = lasteltnode(3);
  setposcaps(N(2),L("last"));	# 04/21/07 AM.
  group(2,3,"_caps");
  group(2,2,"_noun");
  clearpos(N(2),1,0);
  N("cap",2) = 1;
...
@POST
  ...
  if (!L("newsent"))
    ...
    N("cap") = 1;
	N("ne") = 1;
	if (pnname(N(1)) == "_noun" && !N("mypos"))
#	  chpos(N(1),"NP");	# 01/14/05 AM.
	  fixnoun(N(1));
	}
...
@RULES
_xNIL <- #22. 83%
	_xWILD [s one match=(_xCAP) except=(_pro _det _fnword _prep)]
	@@
